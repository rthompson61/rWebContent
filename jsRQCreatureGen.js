...
function updateMissileWeapons(template, params, level){
	try{
		...
		if(template.damageBonus != "none" && template.damageBonus != "0"){
			var halfBonus = "half";
...
function getDressed(t, d, p){
	...
	try{
		...
		if(p[3] == "Trews" && getArmorByLocation(t.body.hitLocations, "abdom", p[2]) != "Hauberk"){
			...
			t = takePantsOff(t,d,p);	
//			window.alert("Test jsRQCreatureGen.getDressed: (1) "+p[1].indexOf("chest")+" | "+p[1].indexOf("rleg"))+ " Params2 "+p[2]+"  Params3 "+p[3];
